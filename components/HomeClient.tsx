...
import { getEntryCoverImage, type ExperienceEntry, localized } from '@/lib/experience'
...
const allHeroImages = [
  {
    src: '/media/593dbeb07aefba23a9ec07aad3892aaa',
    alt: 'MakeNTU Jarvis prototype booth',
    position: '48% 56%',
    zoom: 1.035,
  },
  {
    src: '/media/6d1254d718dc6b88684f4c33771a61e0',
    alt: 'Tyler seated in the NTU Racing race car',
    position: '50% 62%',
  },
  {
    src: '/media/b81104457e73838418d9f57c1b52cb33',
...
export default function HomeClient({ entries }: { entries: ExperienceEntry[] }) {
  const { locale, t } = useTranslation()
  const allowedMedia = new Set(entries.flatMap(entry => [...entry.images, ...entry.evidence].map(image => image.src)))
  const heroImages = allHeroImages.filter(image => allowedMedia.has(image.src))
  const featuredExperiences = entries.filter(entry => entry.featured).slice(0, 4)
  const selectedAreas =
    locale === 'zh'
