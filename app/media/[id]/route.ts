...
import { snapshotPublicAsset, usesPublicSnapshot } from '@/lib/publicSnapshot'
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (usesPublicSnapshot()) {
    const asset = snapshotPublicAsset(id)
    if (!asset) return new Response(null, { status: 404, headers: privateHeaders })
    return new Response(asset.bytes, { headers: { ...privateHeaders, 'Content-Type': asset.mime, 'X-Content-Type-Options': 'nosniff', 'Content-Security-Policy': "default-src 'none'; sandbox", 'Cross-Origin-Resource-Policy': 'same-origin' } })
  }
